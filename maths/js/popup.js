...
link.rel = "stylesheet", link.type = "text/css", link.href = "css/popup.min.css", link.media = "all", head.appendChild(link), link.onload = function() {
...
class Popup {
        ...
        if (!this.params || !this.params.contentAsHtml)
        {
            ...
            for (let t = 0; t < this.content.length; t++) {
                ...
                if ("" !== i) {
                    ...
                    for (i = i.replace(/  /g, "&nbsp;&nbsp;");
                        /{a-(.*?)}\[(.*?)]/.test(i);) i = i.replace(/{a-(.*?)}\[(.*?)]/g, '<a href="$1" target="_blank">$2</a>');
                    for (;
                        /{btn-(.*?)}\[(.*?)]/.test(i);) i = i.replace(/{btn-(.*?)}\[(.*?)]/g, '<button class="$1">$2</button>');
                    i = i.replace(/([^\\]?){/g, '$1<span class="').replace(/([^\\]?)}\[/g, '$1">').replace(/([^\\]?)]/g, "$1</span>"), this.content[t] = i
                }
            }
            this.content = this.content.join("");
        }
        if (this.content, this.popupEl = document.createElement("div"), this.popupEl.classList.add("popup"), this.popupEl.classList.add(this.id), this.popupEl.innerHTML = `\n        <div class="popup-content">\n            <div class="popup-header">\n                ${this.hideTitle?"":`<div class="popup-title">${this.title}</div>`}\n                ${this.allowClose&&!this.hideCloseButton?'<div class="popup-close">&times;</div>':""}\n            </div>\n            <div class="popup-body">${this.content}</div>\n        </div>`, document.body.appendChild(this.popupEl), this.popupEl.addEventListener("click", (t => {
                if ("popup-close" == t.target.className || t.target.classList.contains("popup")) {
                    if (!this.allowClose) return;
                    this.hide()
                }
            })), this.params.loadCallback && "function" == typeof this.params.loadCallback && this.params.loadCallback(), this.showImmediately) {
    ...
    show() {
        ...
        const showCallback = this.params.showCallback;
...
function postHide(t) {
    t.params.hideCallback && "function" == typeof t.params.hideCallback && t.params.hideCallback(t), enableScroll()
}
